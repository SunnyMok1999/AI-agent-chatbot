...
def _llm_for_agent(agent_name: str) -> ChatOpenAI:
    model_by_agent = {
        "NewtonAgent": config.NVIDIA_MODEL_NEWTON,
        "GaussAgent": config.NVIDIA_MODEL_GAUSS,
        "FeynmanAgent": config.NVIDIA_MODEL_FEYNMAN,
        "PolyaAgent": config.NVIDIA_MODEL_POLYA,
        "GriffithsAgent": config.NVIDIA_MODEL_GRIFFITHS,
        "SakuraiAgent": config.NVIDIA_MODEL_SAKURAI,
    }
    return _get_llm(model_by_agent.get(agent_name, config.NVIDIA_MODEL))


def _manager_llm() -> ChatOpenAI:
    return _get_llm(config.NVIDIA_MODEL_MANAGER)


llm = _get_llm(config.NVIDIA_MODEL)
...
def _detect_ambiguity(question: str, intent: Dict[str, Any], has_image: bool = False) -> Dict[str, Any]:
    q = (question or "").strip()
    q_lower = q.lower()
    reasons: List[str] = []
    score = 0.0
...
def node_disambiguate(state: GraphState) -> GraphState:
    ...
    ambiguity = _detect_ambiguity(question=question, intent=intent, has_image=has_image)
...
def node_generate(state: GraphState) -> GraphState:
    ...
    if config.ENABLE_MULTI_AGENT_TUTOR:
        ...
        for agent_name, agent_prompt in selected_experts.items():
            ...
            agent_context = _build_context(agent_docs) if agent_docs else context
