...
function refersToUploadedDocument(text: string): boolean {
  return /\b(upload|uploaded|image|png|pdf|file|document|screenshot|photo|pastpaper|paper)\b/i.test(text);
}
...
export const processRagQuestion = async (question: string, onToken?: (token: string) => void) => {
  ...
  try {
    ...
    if (!latestUpload && !latestImageUpload && refersToUploadedDocument(normalizedQuestion)) {
      answer =
        'I cannot find readable content from your latest upload. If your PDF is scanned/image-only, please upload a PNG screenshot (drag-and-drop supported) or run OCR on the PDF first.';
      if (onToken) {
        onToken(answer);
        return '';
      }
